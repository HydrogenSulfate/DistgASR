...
class Net(nn.Module):
    def __init__(self, angRes_in, angRes_out):
        ...
        self.init_conv = nn.Conv2d(3, channels, kernel_size=3, stride=1, dilation=angRes_in, padding=angRes_in, bias=False)
        self.head_conv = nn.Conv2d(channels * 2, channels, kernel_size=3, stride=1, dilation=angRes_in, padding=angRes_in, bias=False)
    ...
    def forward(self, x, y):
        ...
        y = SAI2MacPI(y, self.angRes_in)
        #  blockshuffle down
        bufferx = self.init_conv(x)
        buffery = self.init_conv(y)
        buffer = torch.cat((bufferx, buffery), dim=1)
        buffer = self.head_conv(buffer)
